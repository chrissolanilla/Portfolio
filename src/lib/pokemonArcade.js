export function startGame({ canvas, buttons }) {
  //sound system
  ...
  function frame() {
    ...
    for (const f of fs) {
      for (let i = 0; i < f.length; ++i) {
        ...
        line(
          screen(
            project(translate_z(translate(rotate_xz(a, angle), cubePos), dz)),
          ),
          screen(
            project(translate_z(translate(rotate_xz(b, angle), cubePos), dz)),
          ),
        );
      }
    }
