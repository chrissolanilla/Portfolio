<script>
  ...
  import { startGame } from '$lib/pokemonArcade.js';

  let game;
  let upBtn, downBtn, leftBtn, rightBtn, aBtn, bBtn, startBtn, selectBtn;
...
<style lang="scss">
  ...
  .hud {
    ...
    /* shared button look */
    button {
      background: rgba(191, 149, 249, 0.75);
      color: #000;
  ...
  /* select and start */
  .meta {
    --meta-h: clamp(36px, 6vmin, 44px);
    --meta-w: clamp(64px, 12vmin, 90px);
  ...
  /* A and B buttons */
  .ab {
    --ab-size: clamp(36px, 10vmin, 74px);
...
<div id="app">
  <!-- BEGINappp shit idk why  -->
  <canvas id="game" bind:this="{game}"> </canvas>
